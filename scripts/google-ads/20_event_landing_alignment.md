...
Prepared and applied September 14, 2026. **All 22 operations were accepted in the supplied live log.** The four new RSAs were created paused. Fresh readback via [Script 20B](20b_verify_activate_event_ads.md) passed. Script 20B activated all four replacements September 15; its final read-only Preview verified four enabled replacements and four paused originals, with zero changes or waiting pairs. See [execution record](20_live_2026-09-14.md).
...
A Preview is a read-only account/configuration check, **not** a server-side validation of the mutations or an ad-policy approval. The corrected Google Ads Preview passed and its full 22-operation plan was reviewed. The subsequent live log confirms acceptance of all 22 mutations. Fresh-state checks subsequently passed in Script 20B. Review and activation completed September 15: Birthday was APPROVED, the other three replacements APPROVED_LIMITED, and all four switched pairs passed final readback. If any expected campaign, ad group, keyword, URL or tracking source differs, the script stops before submitting changes instead of guessing.
...
**Script 20 does not activate the drafts.** The exact-ID verification/activation step, [Script 20B](20b_verify_activate_event_ads.md), completed September 15. It accepted all eight status updates after all four reviews completed; fresh readback verified the replacement enabled and its original paused in every group. The log records each old source ad and every new ad resource name. The originals stayed enabled while replacements awaited review. Ads classified as alcohol information may be Eligible (Limited); that label alone is not a disapproval and is not a reason to misrepresent the offering.
...
The URL/sitelink release applied September 14; its first fourteen complete subsequent Ads account days are **September 15–28**, reviewed September 29 or later. Replacement ads activated September 15; use **September 16–29**, reviewed **September 30 or later**, for their first full fourteen days. Both are separate from Script 19's September 14–27 window. Account timezone is **America/New_York**; align operational reporting. Keep purchases/value, event inquiries, directions and calls separate, and connect Avery's qualified/paid events where attribution permits. Do not treat an inquiry as a confirmed booking or a city winner.

## Rollback

Use saved `PLAN.before` URL records to restore only the changed keyword final/mobile URLs. Remove the five **new ad-group associations** by the logged IDs if needed; do not delete a shared sitelink asset or remove unrelated account/campaign associations. The four replacements are now enabled through Script 20B. If a replacement needs reverting, confirm the original is currently eligible and restore it before pausing the replacement. Bids, budgets and targeting need no rollback from this script.
