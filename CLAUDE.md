...
- **2026-05-03 — Pin Tilt watermark opacity bumped 5% → 12% on `/`.** At 5% the `.snap::before` watermark was technically present but functionally invisible against the indigo background. 12% reads as a present-but-quiet brand pattern without competing with copy. Single one-line change in `index.astro:611`.
