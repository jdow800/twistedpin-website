...
- **`/liquor` is now `/cogs`** (section param, defaults to bar). Any doc still saying `/liquor` is stale. **A "Count food" tile lives there as of 2026-09-07** — a separate walk from the liquor count, its own screen (`views/CountFood.tsx`), all CSS namespaced `lq-fc-*`. See the 2026-09-07 entry in `Context/history/2026-website-decisions.md`. **A "Teacher Group Organizer" tile landed 2026-09-25** (`views/TeacherGroup.tsx`, CSS `lq-tg-*`): staff upload the organizer's sheet, and TPRS resizes the lanes and emails the kitchen/POS packet. See that date's entry.
