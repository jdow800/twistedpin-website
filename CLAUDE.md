...
- **2026-05-03 — Pin Tilt watermark opacity 5% → 12% → 8% on `/`.** At 5% the `.snap::before` watermark was technically present but functionally invisible. Initial bump to 12% read too loud IRL (looked fine in screenshot, felt heavy on the deployed page). Walked back to 8% — present-but-quiet brand pattern that doesn't compete with copy. Single value tweak in `index.astro:611`.
