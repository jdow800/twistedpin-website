...
| GiveMe5 | live | `/feedback` 302s to `www.gm5.ai/twisted-pin` (vendor moved off `app.giveme5.ai` — repo redirect updated 2026-08-07). **If the check-in trigger still texts the raw vendor URL rather than `twistedpin.com/feedback`, it is pointing at the old domain and our redirect does not cover it** — it lives outside this repo. Review platforms typically collect contact and send their own follow-ups. |
