...
**Re-render `/invite-previews/`.** The scratch script `render-themes.cjs` (Eta standalone against `apps/backend/views`, `PREVIEW=1`, 7 looks × 3 shapes) writes `public/invite-previews/<look>--<shape>.html`; commit + push (Vercel auto-deploys). Recreate the script from memory `tprs-invitations-rsvp-build` if the scratchpad is gone — it is ~30 lines: fake `view` per shape, `eta.render("guest/invitation", {...})`.
