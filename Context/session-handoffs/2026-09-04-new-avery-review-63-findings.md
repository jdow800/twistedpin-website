...
## CLOSE-OUT (2026-09-05 ~04:00-05:00 CT)

Walkthrough DIFF (212 real replies, before-commit-60d1638 vs after, same clock; `agents/diff.mjs`): 212/212 rows changed, all but four for the intended reasons; the four exposed the new link-gate ordering bug (message-shaped reasons above eligibility -> a kids party / 5-person group / fundraiser asking a question got the OFFER STILL OPEN re-offer) - fixed + 4 harness cases (WF2 9ad31e3d). Then the three date items: #4 month inherit from notes / summary / Avery's last line in BOTH resolvers; #53 list-neighbour month either direction; #54 an agreeing relative weekday binds (WF2 d456e887; `test-date-resolvers.mjs` 7/7). #47 closed without change (prompt/retry no longer instruct the offer).

**PartiallyPaid - RESOLVED 2026-09-05 (Jon: leave it, not nuclear):** the premise was half wrong. The guest pay page only offers "pay another amount" on FINAL-PAYMENT sessions - `POST /api/m/payment-intents` says "Initial-deposit sessions ignore the body entirely" - so a guest cannot short a deposit link; a partial FIRST payment can only come from staff (an external cash/check record below the deposit, or a staff-chosen smaller deposit amount, which is deliberate). No engine change, no new state. If a staff-recorded partial ever shows up on a pending_deposit booking, the cheap guard is on the admin external-payment form, not in Avery. Original note kept below for the record.

**PartiallyPaid (original note):** TPRS `rail-webhook-handler.ts` ~557 and `payment.ts` ~923 flip `pending_deposit` -> `deposit_paid` on the FIRST successful payment of any amount ("First PaymentRecord transition; partial paid"); the pay page allows partial payments ($25 minimum per KB 1802). So `deposit_paid` (Avery's "PartiallyPaid") means some money landed, not necessarily the full deposit; the balance link collects the rest at day 7 either way. Guest-facing effect: Avery says the deposit landed on a $25 partial. Options if Jon cares: (a) TPRS flips deposit_paid only when paid >= the deposit amount (engine change), (b) Avery says "your payment landed" instead of "deposit" when paid < deposit (needs the paid amount in her context), (c) leave it.
