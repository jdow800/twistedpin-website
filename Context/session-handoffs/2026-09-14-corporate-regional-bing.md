...
- Code commit a7f8cd5 passed both Vercel Preview and Production builds. Production deployment: https://twistedpin-website-4lqp6qgg6-jon-9416s-projects.vercel.app. The public www URLs for all three edited pages returned HTTP 200 and passed the same rendered-content checks. The corporate page also passed WWW/trailing-slash redirect checks, sitemap inclusion and a Bingbot user-agent fetch. The latter is a simulated request, not an actual Bing crawl. The preview URL required Vercel SSO, so preview availability was established through its successful build status; served-content verification used the production domain.

Prepared in an isolated worktree from origin/main. Unrelated work in the original checkout is not part of this release.
