...
| ~~**GoTab menu image load perf**~~ ✅ Shipped 2026-05-13 (commit `0431659`) | — | Route GoTab images through Vercel Image Optimization (`/_vercel/image`). `imageService: true` enabled, `img.gotab.io` on remote-pattern allowlist, `cdnImage()` helper used in cocktails.astro + food.astro. Result: AVIF/WebP encoding + global CDN caching. Quota usage well under Pro tier limits. |
