...
| **GoTab menu image load perf** — flagged 2026-05-13 smoke test | Anytime | Cocktail + food menus pull product images directly from GoTab; load felt slow. Options: (1) Vercel `<Image>` component proxy for auto-AVIF/WebP + lazy-loading; (2) build-time download + local re-encode into `/public/menu/`; (3) ask GoTab to serve compressed variants. Low priority — menu pages aren't LCP-critical. |
| **🔥 Review Bot Protection logs → flip from Logging to Enforcement** | **Day 7** (2026-05-20) | Vercel project → Firewall → Bot Management. Bot Protection is currently in **logging mode** (enabled 2026-05-13 at launch). Review the log to confirm only obvious bots are being flagged — no legit customer traffic, no Googlebot, no Bingbot. If clean: flip toggle from Logging → On. If false positives surfaced: investigate before flipping. **Set a calendar reminder for 2026-05-20.** |
| **Day 30 — geo-restriction review** | Day 30 (2026-06-12) | Look at Web Analytics → Geography breakdown. If meaningful malicious foreign traffic visible, add Vercel Firewall custom rule to block Russia + China + North Korea + Iran (Tier 2 plan). If traffic looks clean, no action needed. Aggressive USA-only blocking deferred indefinitely — risks blocking SEO crawlers + traveling customers + AI search bots. |
...
- **2026-05-13 — Launch executed.** Worktree merged + pushed (commit `5eee826`). Vercel DNS zone populated with 11 records. Nameservers flipped at GoDaddy from Cloudflare to `ns1/ns2.vercel-dns.com`. Site loads from cellular ✅. Email auth verified for both paths: Freshdesk outbound (selector `zhdjb`, SPF/DKIM/DMARC all PASS) AND Google Workspace outbound to Yahoo (selector `google`, SPF/DKIM/DMARC all PASS). DNS propagation ~76% complete at +15 min (whatsmydns NS check). Local laptop DNS cache still serving old site — expected, will self-resolve overnight as ISP DNS caches expire.
- **2026-05-13 — Bot Protection enabled in logging mode.** Vercel project → Firewall → Bot Management → Bot Protection: ON in **logging mode** (challenges nothing, only logs what WOULD be blocked). Day-7 calendar check planned: review logs, confirm only obvious bots flagged, then flip to enforcement. **AI Bots: deliberately kept OFF** — venue benefits from being discoverable in ChatGPT/Claude/Perplexity/Google AI Overviews answers to "Plainfield event venue" type queries; bandwidth cost negligible at Pro tier; schema markup work specifically targets AI consumption. **Geo-restriction: deferred to Day 30 review** — wait to see actual traffic mix before deciding. Tiered plan documented: Tier 1 (no restrictions, default) → Tier 2 (block 4 high-abuse countries) → Tier 3 (USA+Canada with bot allow-list, challenge mode) → Tier 4 (USA-only hard block, NOT recommended).
