...
- **/new-years-eve + /reserve/nye** — NYE surfaces all lean on the generic VIP suite shot (2026-06-11): the lander's og:image and the booking page's hero band + og reuse `vip energy.jpg` / `og-vip-suite.jpg`. **Need: real NYE party photography from the 2026-12-31 night** (confetti, midnight toast, the suite mid-party) → dedicated 1200×630 og crop for the lander, hero re-encode for `/reserve/nye`, and heroImageUrl uploads on TPRS products 124/126 (powers the booking page's product-hero takeover). Until then the suite shot is a fine stand-in — just visually redundant with `/vip-suite`
