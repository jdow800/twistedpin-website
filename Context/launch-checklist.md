...
| **Patch Retention API** (coupon signup) | **Shipped 2026-05-05.** `/coupon` ships a native form that POSTs to `/api/coupon-signup` (server endpoint) which upserts into Patch via `PATCH /v2/contacts?match:phone={phone}`. Replaces the legacy iframe. API key + Account ID kept server-side. | **Set 3 env vars on Vercel:** `PATCH_API_KEY` (Bearer token from Patch dashboard), `PATCH_ACCOUNT_ID` (Account ID — sent as `X-Account-Id` header on every request, REQUIRED alongside the Bearer token), and `PATCH_TAG` (tag name applied to new signups, e.g. `coupon_10_lane` — Patch dashboard automation triggers off this tag to send the SMS with the actual coupon code). Until `PATCH_API_KEY` and `PATCH_ACCOUNT_ID` are set, the page returns a "temporarily unavailable" message. Also verify in Patch dashboard: birthday custom field exists (we send as `birthday: "MM-DD"`) — if Patch expects a different key, adjust `src/pages/api/coupon-signup.ts`. |
