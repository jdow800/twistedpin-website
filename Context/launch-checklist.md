...
| **Plan-an-Event platform (Heyflow / Zite toggle)** | **Toggle infrastructure shipped 2026-05-12** (`src/lib/links.ts` — `PLAN_EVENT_URL` switches between `AVERY_OFF_URL` Heyflow `event.twistedpin.com/#start` and `AVERY_ON_URL` Zite `twistedevents.zite.so`). 19 CTAs import the constant; one-line flip propagates everywhere in ~90s. **Currently set to Heyflow (OFF)** — matches ops reality and ads sitelinks (2026-05-17). Avery (Zite's AI sales agent) is a theory under test; the toggle pattern is "turn it on, evaluate, turn it off to troubleshoot." Long-form blog/llms.txt copy is NOT auto-toggled — flipped manually 2026-05-17 to match. | When permanent platform decision lands: update the export at `src/lib/links.ts:30` and (only if the new platform is permanent) sweep blog/llms.txt URLs to match. |
...
- [x] `/coupon` — shipped 2026-05-05 as a native form, **reverted to iframe wrapper 2026-05-07**. Patch trigger pattern never resolved; user opted to ship the legacy live form via `c-g.co/xORo1J` embed. Voice reframe ("$10 OFF" → "$10 lane credit") preserved. `src/pages/api/coupon-signup.ts` deleted. `PATCH_API_KEY` + `PATCH_ACCOUNT_ID` env vars can be removed from Vercel
...
- [ ] **Rich Results Test sanity pass** — run one URL of each unique schema type through [Google Rich Results Test](https://search.google.com/test/rich-results) and confirm zero errors. Catches schema typos build-time validation can't. Suggested set (~10 min total): `/` (BarOrPub + WebSite), `/bar/` (BarOrPub + Person + BreadcrumbList), `/menu/cocktails/` (Menu + 3-crumb BreadcrumbList), `/why-us/naperville-il/` (Service + BreadcrumbList), `/blog/things-to-do-plainfield-il/` (Article + BreadcrumbList), `/faq/` (FAQPage + BreadcrumbList), `/new-years-eve/` (Event + BreadcrumbList).
